...
int main()
{
	const auto memory = Memory{ "Valorant.exe" };

	//Obetener las direcciones de los modulos
	...
	while (true) {
		...
		for (const auto& handle : weapons) {
			...
			{
				const bool ShouldUpdate = memory.Read<std::int64_t>(weapon + offset::m_nFallbackPaintKit) != paint;
			}
		}
	}
